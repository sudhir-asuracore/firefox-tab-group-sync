...
async function syncGroupsFromRemote(groupsToSync, options = {}) {
  ...
  for (const remoteGroup of groupsToSync) {
    ...
    if (mirror) {
      ...
      const tabsToRemoveIds = existingTabs
      ...
      const tabsToCreate = normalizedRemoteTabs.filter(url => !existingUrls.has(url));
